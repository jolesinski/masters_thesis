...
In addition to the spatial information contained in RGB-D images, an important and discriminative function is also performed by the description of texture. Although there are descriptor proposals in literature that combine both information cues \cite{CSHOT, RGBFPFH}, the best performance is achieved when color is analyzed independently of shape. The benefits are in further matching phase, where descriptors of smaller sizes are compared in the same information domain and there are overall more keypoints for grouping and generating pose hypotheses.

In comparison to shape, texture descriptors are more broadly studiend and developed field. A significant interest in this area was aided by numerous successful applications \cite{ORB} of the \textit{Scale-invariant Feature Transform} (SIFT) \cite{SIFT}.

Take description from Scholarpedia SIFT

A common technique shared among these methods is the \textit{scale pyramid}.
